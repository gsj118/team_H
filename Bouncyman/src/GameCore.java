...
public class GameCore {
     ...
     */
    private static class GamePanel extends JPanel implements Runnable, KeyListener {
        ...
        private void renderTutorialOverlay(Graphics2D g) {
            ...
            g.fillRect(80, 60, WIDTH - 160, 190);
            ...
            g.drawString("Space : 보석 능력 사용 (노란: 대시 / 파란: 고점프)", x, y); y += dy;
            g.drawString("바닥에 닿으면 자동으로 튕겨 오릅니다.", x, y); y += dy;
            g.drawString("노란 별을 모두 먹으면 문제 단계로 넘어갑니다.", x, y); y += dy;
        ...
        private void renderEndingOverlay(Graphics2D g) {
            ...
            String title = StageStory.getEndingTitle(currentStageIndex, lastAnswerCorrect);
            ...
            String[] lines = StageStory.getEndingLines(currentStageIndex, lastAnswerCorrect);
            g.setFont(g.getFont().deriveFont(Font.PLAIN, 14f));
            ...
            StageInfo info = stageInfos[currentStageIndex];
            if (info != null) {
                g.drawString("별 수집: " + info.getCollectedStars() + " / " + info.getTotalStars(), x, y);
                y += dy;
                g.drawString("사망 횟수: " + info.getDeathCount(), x, y);
                y += dy;
                g.drawString("클리어 시간: " + String.format("%.2f초", info.getClearTimeSeconds()), x, y);
                y += dy;
            }

            y += 10;
            g.drawString("Enter 키를 눌러 다음 스테이지로 진행합니다.", x, y);
        }
